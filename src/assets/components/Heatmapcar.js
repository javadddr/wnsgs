  ...
  useEffect(() => {
    if (filteredData.length > 0) {
      const generateHeatmapChart = () => {
        ...
        const names = Array.from(new Set(filteredData.map((item) => item['Name2'])));
        ...
        const groupedData = names.map((name) => {
          const filteredGroup = filteredData.filter((d) => d['Name2'] === name);
        ...
        svg.selectAll('.label')
          ...
          .append('text')
          .attr('class', 'label')
          .attr('x', (d) => xScale(d.Name2) + xScale.bandwidth() / 2)
  ...
  return (
    <div className="heatmapi1">
      <h1 style={{ fontSize: '16px', margin: '10px', marginLeft: '30px' }}>
        Average selling prices per kg for each metal vs Beevans
      </h1>
