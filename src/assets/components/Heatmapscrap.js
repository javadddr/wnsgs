  ...
  useEffect(() => {
    if (filteredData.length > 0) {
      const generateHeatmapChart = () => {
        ...
        const weekNumbers = Array.from(new Set(filteredData.map((item) => item.weekNumber)));
        ...
        const groupedData = weekNumbers.map((weekNumber) => {
          const filteredGroup = filteredData.filter((d) => d.weekNumber === weekNumber);
        ...
        const xScale = d3.scaleBand().range([0, width]).domain(weekNumbers).padding(0.1);

        const yScale = d3.scaleBand().range([0, height]).domain(metals).padding(0.1);
        ...
        svg.selectAll('.label')
          ...
          .append('text')
          .attr('class', 'label')
          .attr('x', (d) => xScale(d.weekNumber) + xScale.bandwidth() / 2)
